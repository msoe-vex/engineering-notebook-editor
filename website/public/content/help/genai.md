...
2. Under **Generative AI**, pick a provider: **Gemini**, **OpenAI**, or **Claude**.
3. Paste an API key for that provider. Keys stay in **this browser only**. They are not saved in the notebook or in Git.
4. Optionally type a **model id**. Leave it blank to use the built-in default for that provider (currently `gemini-3.5-flash-lite` for Gemini).
...
- **Your API key’s account** is billed by Gemini, OpenAI, or Anthropic.
